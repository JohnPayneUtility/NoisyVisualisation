...
| `test_mo_umda_kmeans.py` | The clustering `MoUMDA_KMeans` contract, in-process, with K-means replaced by fixed labels wherever an invariant is checked: λ = 2μ and k = ⌊√μ⌋ validation (rejected before any evaluation), rejected `starting_solution`/margin/duplicate options; Bernoulli(0.5) initial population regardless of `attr_function`; NSGA-II selection of μ; K-means receives the (μ, M) stored noisy objectives, never genotypes or true fitness, with no evaluations; per-cluster margin-free genotype frequencies and `2*q_i` samples per cluster (empty clusters sample none), duplicates allowed; full replacement with λ evaluations per generation and commit-on-success; only the generic stop criteria apply; the explicit K-means settings; identically seeded runs are identical; the default config runs through the MO runner's per-seed function |
