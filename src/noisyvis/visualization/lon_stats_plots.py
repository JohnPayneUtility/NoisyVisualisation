...
def plot_lon_scatter_multi(node_stats_by_series: Dict[Any, List[Dict[str, Any]]], x_key: str, y_key: str) -> go.Figure:
    """
    Scatter plot of one LON node quantity against another, with one trace
    per noise-intensity series (see plot_lon_stats_multi).

    Args:
        node_stats_by_series: Mapping of series label (e.g. noise intensity)
            to compute_node_feasibility_error() output for that series
        x_key: Key into each node_stats dict to use as x (see AXIS_OPTIONS)
        y_key: Key into each node_stats dict to use as y (see AXIS_OPTIONS)
    ...
    if not any(node_stats_by_series.values()):
        fig.update_layout(title="No data - select NLon_box or NLon_IQR with a loaded LON")
        return fig

    colors = px.colors.qualitative.Plotly
    for i, series_label in enumerate(sorted(node_stats_by_series.keys())):
        node_stats = node_stats_by_series[series_label]
        if not node_stats:
            continue
        ...
        y = [s[y_key] for s in node_stats]
        hover_text = [
            f"{s['node']}<br>Noise intensity: {series_label}<br>Fitness: {s['fitness']:.3f}"
            f"<br>Median sample: {s['median']:.3f}<br>Error: {s['error']:.3f}<br>Sample range: {s['iqr']:.3f}"
            for s in node_stats
        ]

        fig.add_trace(go.Scatter(
            x=x, y=y, mode='markers',
            marker=dict(size=8, opacity=0.75, color=colors[i % len(colors)]),
            text=hover_text,
            hoverinfo='text',
            name=f"Noise intensity {series_label}",
        ))
    ...
    fig.update_layout(
        title=f"{x_label} vs. {y_label} (by noise intensity)",
        xaxis_title=x_label,
        yaxis_title=y_label,
        showlegend=True,
    )
...
def plot_lon_violin_multi(node_stats_by_series: Dict[Any, List[Dict[str, Any]]], x_key: str, y_key: str) -> go.Figure:
    """
    Violin plot of y grouped by each unique x value, with one violin series
    per noise-intensity series (see plot_lon_stats_multi).
    ...
    fig.update_layout(
        title=f"{x_label} vs. {y_label} (grouped by unique {x_label} values, by noise intensity)",
        xaxis_title=x_label,
        yaxis_title=y_label,
        violinmode='group',
        showlegend=True,
...
def plot_lon_stats_multi(
    node_stats_by_series: Dict[Any, List[Dict[str, Any]]],
    x_key: str,
    y_key: str,
    plot_style: str = DEFAULT_PLOT_STYLE,
) -> go.Figure:
    """
    Dispatch to the scatter or violin multi-series LON node plot based on
    plot_style. Each entry in node_stats_by_series becomes its own trace
    (e.g. one per sampled noise intensity).

    Args:
        node_stats_by_series: Mapping of series label to
            compute_node_feasibility_error() output for that series
        x_key: Key into each node_stats dict to use as x (see AXIS_OPTIONS)
        y_key: Key into each node_stats dict to use as y (see AXIS_OPTIONS)
        plot_style: 'scatter' or 'violin'
    ...
    """
    if plot_style == 'violin':
        return plot_lon_violin_multi(node_stats_by_series, x_key, y_key)
    return plot_lon_scatter_multi(node_stats_by_series, x_key, y_key)
