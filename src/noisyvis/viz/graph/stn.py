"""STN graph population for the shared display graph.

The Search-Trajectory-Network half of the former `visualization/graph_builder.py`. These functions
populate the single `nx.MultiDiGraph` that `noisyvis.dashboard` builds per callback; the LON half
lives in `noisyvis.viz.graph.lon` and populates the same graph object. Node ids, node attributes and
edge attributes written here are contracts read by `noisyvis.viz.layout`, `noisyvis.viz.styling`,
`noisyvis.viz.traces`, `noisyvis.analysis.graph_stats` and the dashboard's annotation logic.
"""
...
def add_stn_trajectories(
    ...
    for run_idx, entry in enumerate(all_run_trajectories):
        ...
        # Extract fitness boxplot stats (index 6) and estimated fitness data (indices 8-11)
        fitness_boxplot_stats = entry[6] if len(entry) > 6 else []
        ...
        for i, solution in enumerate(unique_solutions):
            ...
            if key not in stn_node_mapping:
                ...
                G.add_node(
                    ...
                    fitness_boxplot_stats=boxplot_stat,
...
def add_prior_noise_stn_v4(
    ...
    Entry format (8 elements):
        [rep_sols, rep_fits, rep_noisy_fits, sol_iterations,
         sol_transitions, _, _, rep_noisy_sols]
    """
    for run_idx, entry in enumerate(all_run_trajectories):
        if len(entry) < 8:
